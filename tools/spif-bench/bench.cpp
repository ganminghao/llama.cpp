...
PrefData RecordPrefData(struct llama_context * ctx){
    PrefData data;
    llama_perf_context_data perf_data = llama_perf_context(ctx);
    data.n_prefll    = perf_data.n_p_eval;
    data.t_prefll_ms = perf_data.t_p_eval_ms;
    data.n_decode    = perf_data.n_eval;
    data.t_decode_ms = perf_data.t_eval_ms;
    return data;
}

void printPrefData(std::vector<PrefData> pref_data){
    ...
    for (size_t i = 0; i < pref_data.size(); ++i) {
    ...
    double avg_decode_throughput = total_decode_tokens / (total_decode_time / 1000.0); // tokens per second
    LOG("\nTotal:\n");
    LOG("prefill throughput = %.2f t/s, decode throughput = %.2f t/s\n",
        avg_prefill_throughput, avg_decode_throughput);

    LOG("\n\n");
}

std::vector<std::string> load_prompts_from_file(std::string path, int n_prompts) {
    ...
    std::string line;
    int cnt = 0;
    while (std::getline(file, line) && (cnt < n_prompts || n_prompts == -1)) {
        if (!line.empty()) {
...
int main(int argc, char ** argv) {
    ...
    // std::string prompt;
    std::vector<std::string> prompts;
    prompts = load_prompts_from_file(params.prompt_file, params.n_prompts);
    std::vector<std::vector<llama_token>> prompt_tokens;

    std::vector<PrefData> pref_datas;
    ...
    for (int prompt_idx = 0; prompt_idx < (int) prompt_tokens.size(); prompt_idx++) {
        ...
        while ((n_remain != 0 && !is_antiprompt) || params.interactive) {
            ...
            if ((int) embd_inp.size() <= n_consumed) {
                ...
                if (params.conversation_mode && !waiting_for_first_input) {
                    if (!prompts[prompt_idx].empty()) {
                        prompts[prompt_idx].clear();
        ...
        // record data
        pref_datas.push_back(RecordPrefData(ctx));
