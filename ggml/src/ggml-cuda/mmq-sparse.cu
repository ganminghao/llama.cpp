...
static __device__ __forceinline__ float dot_block_q8_0_q8_1_dp4a(
    const block_q8_0 & bx,
    const block_q8_1 & by) {
...
#pragma unroll
    for (int i = 0; i < QK8_0; i += 4) {
        int ax =  (int)(uint8_t)x_qs8[i + 0]
                | ((int)(uint8_t)x_qs8[i + 1] << 8)
                | ((int)(uint8_t)x_qs8[i + 2] << 16)
                | ((int)(uint8_t)x_qs8[i + 3] << 24);

        int ay =  (int)(uint8_t)y_qs8[i + 0]
                | ((int)(uint8_t)y_qs8[i + 1] << 8)
                | ((int)(uint8_t)y_qs8[i + 2] << 16)
                | ((int)(uint8_t)y_qs8[i + 3] << 24);
...
static __global__ void mul_mat_vec_sparse_q8_0_q8_1(
...
static void launch_mul_mat_cuda_sparse_q(
    ...
    if (src1_ncols == 1) {
        ...
        case 32:
            mul_mat_vec_sparse_q8_0_q8_1<32><<<grid, 32, smem, stream>>>(
                w_q8, x_q8, sparse_idx, gpu_neu_idx, dst,
                nrows, num_gpu_neurons, num_blocks);
            break;
        case 64:
            mul_mat_vec_sparse_q8_0_q8_1<64><<<grid, 64, smem, stream>>>(
                w_q8, x_q8, sparse_idx, gpu_neu_idx, dst,
                nrows, num_gpu_neurons, num_blocks);
            break;
        case 96:
            mul_mat_vec_sparse_q8_0_q8_1<96><<<grid, 96, smem, stream>>>(
                w_q8, x_q8, sparse_idx, gpu_neu_idx, dst,
                nrows, num_gpu_neurons, num_blocks);
            break;
        case 128:
            mul_mat_vec_sparse_q8_0_q8_1<128><<<grid, 128, smem, stream>>>(
                w_q8, x_q8, sparse_idx, gpu_neu_idx, dst,
                nrows, num_gpu_neurons, num_blocks);
            break;
        case 160:
            mul_mat_vec_sparse_q8_0_q8_1<160><<<grid, 160, smem, stream>>>(
                w_q8, x_q8, sparse_idx, gpu_neu_idx, dst,
                nrows, num_gpu_neurons, num_blocks);
            break;
        case 192:
            mul_mat_vec_sparse_q8_0_q8_1<192><<<grid, 192, smem, stream>>>(
                w_q8, x_q8, sparse_idx, gpu_neu_idx, dst,
                nrows, num_gpu_neurons, num_blocks);
            break;
        case 224:
            mul_mat_vec_sparse_q8_0_q8_1<224><<<grid, 224, smem, stream>>>(
                w_q8, x_q8, sparse_idx, gpu_neu_idx, dst,
                nrows, num_gpu_neurons, num_blocks);
            break;
        case 256:
        default:
            mul_mat_vec_sparse_q8_0_q8_1<256><<<grid, 256, smem, stream>>>(
                w_q8, x_q8, sparse_idx, gpu_neu_idx, dst,
                nrows, num_gpu_neurons, num_blocks);
            break;
        }
    ...
    switch (block_size_best) {
    case 32:
        mul_mat_batch_sparse_q8_0_q8_1<32><<<grid, 32, smem, stream>>>(
            w_q8, x_q8, sparse_idx, gpu_neu_idx, dst,
            ncols, nrows, src1_ncols, num_gpu_neurons);
        break;
    case 64:
        mul_mat_batch_sparse_q8_0_q8_1<64><<<grid, 64, smem, stream>>>(
            w_q8, x_q8, sparse_idx, gpu_neu_idx, dst,
            ncols, nrows, src1_ncols, num_gpu_neurons);
        break;
    case 96:
        mul_mat_batch_sparse_q8_0_q8_1<96><<<grid, 96, smem, stream>>>(
            w_q8, x_q8, sparse_idx, gpu_neu_idx, dst,
            ncols, nrows, src1_ncols, num_gpu_neurons);
        break;
    case 128:
        mul_mat_batch_sparse_q8_0_q8_1<128><<<grid, 128, smem, stream>>>(
            w_q8, x_q8, sparse_idx, gpu_neu_idx, dst,
            ncols, nrows, src1_ncols, num_gpu_neurons);
        break;
    case 160:
        mul_mat_batch_sparse_q8_0_q8_1<160><<<grid, 160, smem, stream>>>(
            w_q8, x_q8, sparse_idx, gpu_neu_idx, dst,
            ncols, nrows, src1_ncols, num_gpu_neurons);
        break;
    case 192:
        mul_mat_batch_sparse_q8_0_q8_1<192><<<grid, 192, smem, stream>>>(
            w_q8, x_q8, sparse_idx, gpu_neu_idx, dst,
            ncols, nrows, src1_ncols, num_gpu_neurons);
        break;
    case 224:
        mul_mat_batch_sparse_q8_0_q8_1<224><<<grid, 224, smem, stream>>>(
            w_q8, x_q8, sparse_idx, gpu_neu_idx, dst,
            ncols, nrows, src1_ncols, num_gpu_neurons);
        break;
    case 256:
    default:
        mul_mat_batch_sparse_q8_0_q8_1<256><<<grid, 256, smem, stream>>>(
            w_q8, x_q8, sparse_idx, gpu_neu_idx, dst,
            ncols, nrows, src1_ncols, num_gpu_neurons);
        break;
    }
...
void ggml_cuda_mul_mat_sparse_q(
        ggml_backend_cuda_context & ctx,
        const ggml_tensor * src0,   // Q8_0 权重
...

    {
        ...
        quantize_row_q8_1_cuda(
            src1_d, nullptr,
            src1_q8_1.ptr, src1->type,
            ne10, s11, s12, s13,        // ne00, s01, s02, s03
    ...
    const mmq_sparse_args args = {
        ...
        ne00, ne01, ne1, s01,                         // ncols_x, nrows_x, ncols_dst, stride_row_x
        src1_ncols, s1,                               // ncols_y, nrows_dst
        ne02, ne12, s02, nblocks_per_col, s2,         // channel-related
        ne03, ne13, s03,                              // sample-related
        0, s3,
        false, ne1,
        sparse_idx, gpu_neu_idx, num_gpu_neurons
    };
