...
static void launch_mul_mat_axpy_cuda_sparse_rowwise(const T *       x,
    ...
    if (src_ncols == 1) {
        ...
        mul_mat_axpy_sparse_rowwise<T, type_acc>
            <<<block_nums, block_dims, share_mem_size, stream>>>(x, y, dst, ncols, nrows, src_ncols, gpu_neu_idx, sparse_idx);
}
...
static void mul_mat_axpy_cuda_sparse(const T *       x,
    ...
    if (!k_spif_fast_axpy) {
        ...
        // launch_mul_mat_axpy_cuda_sparse_rowwise_pro<T>(x, y, sparse_idx, gpu_neu_idx, dst, ncols, nrows,
